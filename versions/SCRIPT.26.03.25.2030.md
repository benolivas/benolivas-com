...
### EXACT TRIGGERS
*Checked before normalization, before everything. Must match full input exactly (trimmed, case-insensitive).*
*Use for easter eggs, passwords, hidden paths.*

=== EXACT: open_sesame ===
EXACT TRIGGER: "open sesame"

**MEMEBOT:** [image: open-sesame.gif]
// asset: assets/memes/open-sesame.gif — not yet sourced

> **BEN OS:** ...how did you know that.
...
=== INTENT: distress ===
TRIGGERS: i wanna die, want to die, kill myself, end it all, i hate myself,
i want to disappear, suicidal, i'm worthless, i give up completely
...
// AUTHOR'S NOTE: No real generation. Pull from prerendered assets/generated/ folder,
// no repeats per session. Assets should loosely match common prompt patterns —
// abstract, portrait, landscape, product, etc. User thinks they prompted it.
...
// AUTHOR'S NOTE: Whatever the user says here, pick the closest prerendered asset
// and present it as if generated. Categories: abstract, portrait, dark, colorful,
// minimal, chaotic. Map user words to categories, serve asset, never break the illusion.
...
CHIPS:
  ...
  "Let's make something real." → available

---

=== INTENT: generation_interactive ===
NESTED TRIGGERS (generation_flow): interactive, something interactive,
something i can play with, a widget, something fun, interactive thing
...
### INLINE COMPONENTS
Components that render inline within the conversation flow. Always need a `>` if on their own line.

| Annotation | What it renders |
|---|---|
| `> [GIF: path/to/file.gif]` | Single gif inline, plays on load |
| `> [TOGGLE: dark_mode]` | Small unlabeled neumorphic toggle, inline |
| `> [MEDIA: grid-name]` | Image grid (3 or 6 thumbnails) |
| `[LINK: url \| display text]` | Clickable inline link, opens new tab |

---

### INTERACTION BEATS
`*(on interaction)*` — beat does not auto-fire. Waits for user to interact with the previous component (click a toggle, tap an image, etc.) before continuing.
...
### AUTHOR NOTES
- `// short note` — one-liner instruction to Claude, never rendered
- Multi-line notes use `// AUTHOR'S NOTE:` on the first line

---

### EXACT TRIGGERS
`EXACT TRIGGER: "phrase"` — matches the full raw input exactly (trimmed, case-insensitive). Fires before everything else including vignettes. Used for easter eggs and passwords.

---

### STATE & NESTED TRIGGERS
- `STATE: state_name` — sets current state when this intent fires
- `NESTED TRIGGERS (state_name): phrase, phrase` — only match when in that state
- `NESTED TRIGGERS (state_name): *` — wildcard, catches any input in that state

---

### FALLBACKS
`=== FALLBACK: fallback_id ===` — fires automatically when something fails (broken image, no intent match, etc.). Not triggered by user input.
...
*Written per-speaker so the voice stays consistent even in failure.*
...
**Not yet built — engine work needed:**
- `generation_response` — [matched-asset] wildcard logic, category mapping
- `generation_interactive` — [TOGGLE] inline component, *(on interaction)* beat type
- `generate_explain` — intent not yet written
- `generate_save` — intent not yet written
- `[LINK:]` — inline link renderer not yet built
- `[GIF:]` — single inline gif renderer not yet built
- `FALLBACKS` — broken image detection not yet wired
- `distress` — response not yet written
- `dark mode toggle` — neumorphic toggle component in header not yet built
- STATE system — NESTED TRIGGERS engine not yet built

**Unassigned:**
- `drake-little-yachty-oh.gif` — in meme library, waiting for the right moment
- `open-sesame.gif` — exact trigger asset, not yet sourced
